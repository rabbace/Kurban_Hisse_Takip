    ...
    <div className="min-h-screen">
        ...
        <div className="mx-auto max-w-6xl px-4 text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-6 text-sm">
            <div className="flex items-center gap-2">
              <span>✅</span>
              <span>Sağlık sertifikalı hayvanlar</span>
            </div>
            <div className="flex items-center gap-2">
              <span>📍</span>
              <span>Adrese teslim seçeneği</span>
            </div>
            <div className="flex items-center gap-2">
              <span>📱</span>
              <span>Anlık takip sistemi</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-wrap gap-2">
            {animalTypes.map((type) => (
              ...
              >
                {type === ALL_FILTER
                  ? "Tümü"
                  : ANIMAL_TYPE_LABELS[type as AnimalType] ?? type}
              </button>
            ))}
          </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        {loading ? (
          ...
          <div className="py-20 text-center text-gray-500">
            ...
            <p className="text-sm">Farklı bir filtre deneyin</p>
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {filtered.map((animal) => (
      ...
      <footer className="mt-16 border-t border-gray-200 bg-white py-8">
        <div className="mx-auto max-w-6xl px-4 text-center text-sm text-gray-500">
          <p className="font-semibold text-gray-700">Kurban Hisse Takip Sistemi</p>
          <p className="mt-1">
            Sorularınız için{" "}
            <a href="tel:+905001234567" className="text-emerald-600 hover:underline">
              0500 123 45 67
            </a>
          </p>
        </div>
